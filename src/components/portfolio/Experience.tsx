...
export default function Experience() {
  return (
    <section id="experience" className="px-6 md:px-12 py-16 md:py-24 bg-[#FAFAFA]">
      <Reveal>
        <p className="text-[0.62rem] tracking-[0.3em] uppercase text-[#C8A96E] mb-3">02 — Experience</p>
      </Reveal>
      <Reveal delay={100}>
      ...
      <div className="pl-5 md:pl-6 border-l border-[#E0DDD6]">
        {EXPERIENCE.map((item, i) => (
          <Reveal key={i} delay={i * 80}>
            <div className="relative pl-6 md:pl-8 pb-8 md:pb-10 last:pb-0">
              <div className="absolute -left-[1.85rem] md:-left-[2.19rem] top-1.5 w-2 h-2 rounded-full bg-[#C8A96E]" />
              <p className="text-[0.65rem] tracking-[0.15em] uppercase text-[#888] mb-1">{item.date}</p>
              <p className="text-[1.2rem] md:text-[1.45rem] font-normal text-[#0D0D0D] mb-0.5" style={{ fontFamily: "'Cormorant Garamond', serif" }}>
                {item.role}
                {item.current && (
